...
void DrawPlot(TCanvas* c, HistSet& hs, map<string, Process>& processes, bool logY = false) {
    c->cd();
    // Create two pads: one for the main plot, one for the ratio
    ...
    TH1F* h_bkg_total = nullptr; // For ratio plot

    // Define the order for stacking
    vector<string> stack_order = {"higgs", "diboson", "singletop", "wjets", "ztautau", "ttbar", "fake"};
    ...
    // Statistical uncertainty band on total background
    ...
    // Y-axis range
    double ymax = max(h_data->GetMaximum(), stack->GetMaximum());
    stack->SetMaximum(ymax * (logY? 50.0 : 1.5));
    stack->SetMinimum(logY? 0.1 : 0);
    stack->GetYaxis()->SetTitle("Events");
    stack->GetYaxis()->SetTitleOffset(1.5);
    stack->GetXaxis()->SetLabelSize(0); // Hide labels on top plot
    ...
    latex.DrawLatex(0.32, 0.85, "Internal");
    latex.SetTextSize(0.04);
    latex.DrawLatex(0.2, 0.78, "#sqrt{s} = 13 TeV");
    ...
    h_ratio->GetYaxis()->SetLabelSize(0.1); // FIX: SetLabelSize (capital S)
    h_ratio->GetYaxis()->SetTitleOffset(0.5);

    h_ratio->GetXaxis()->SetTitle(hs.xlabel.c_str());
    h_ratio->GetXaxis()->SetTitleSize(0.12);
    h_ratio->GetXaxis()->SetLabelSize(0.1); // FIX: SetLabelSize (capital S)
...
// ==========================================================================================
void validation_plots_stacked_fixed() {
    SetATLASStyle();
    ...
    processes["higgs"] = {"higgs", "Higgs", {base_path + "higgs/*.root"}, TColor::GetColor("#1ABC9C")}; // Turquoise

    vector<string> process_names = {"data", "fake", "ttbar", "ztautau", "wjets", "singletop", "diboson", "higgs"};
  ...
  for (const auto& spec : var_specs) {
    ...
    // (fake-syst variation histos removed — keep only statistical band)
    ...
    for (const auto& proc_pair : processes) {
        ...
        for (Long64_t i = 0; i < nMC; i++) {
            ...
            if (proc_name == "ttbar") {
                mc_scale_factor = 0.48;
            } else if (proc_name == "ztautau") {
                mc_scale_factor = 0.58;
            }
    ...
    // Reset trigger match pointers before switching to data tree — they pointed into
    // the now-deleted MC trees. SafeSet-guarded branches will stay nullptr if absent in data.
    EventData evData;
    ResetTrigMatchPointers(evData);

    TChain *dataChain = new TChain("tree");
    dataChain->Add("../AMM/data_with_electron_fake_weights.root");
    SetupBranches(dataChain, evData);

    Long64_t nData = dataChain->GetEntries();
    for (Long64_t i = 0; i < nData; i++) {
        ...
        double w_fake_nom   = evData.fw_nom;
  ...
  for (auto const& [var_name, hist_set] : histos_VR) {
    ...
    else if (var_name == "l2_eta") val = std::abs(evData.l2_eta);

    hist_set.histograms.at("data")->Fill(val, 1.0);
    hist_set.histograms.at("fake")->Fill(val, w_fake_nom);
  }
}
...
double total_vr  = mc_yields["OS_VR"] + fake_vr;
double fake_vr4  = histos_VR4["MET"].histograms["fake"]->Integral();
double total_vr4 = mc_yields["VR4"] + fake_vr4;
...
printf("+--------------------+-----------------------+-----------------------+\n");
    ...
    system("mkdir -p outputs/03_amm_validation");
    TFile* outFile = new TFile("outputs/03_amm_validation/amm_validation_plots.root", "RECREATE");

    for (auto& pair : histos_VR) {
        TCanvas *c = new TCanvas((string("c_") + pair.first + "_VR").c_str(), "", 800, 800);
        DrawPlot(c, pair.second, processes, true);
        c->SaveAs((string("outputs/03_amm_validation/VR_") + pair.first + ".pdf").c_str());
        c->Write();
    }
    for (auto& pair : histos_VR4) {
        TCanvas *c = new TCanvas((string("c_") + pair.first + "_VR4").c_str(), "", 800, 800);
        DrawPlot(c, pair.second, processes, true);
        c->SaveAs((string("outputs/03_amm_validation/VR4_") + pair.first + ".pdf").c_str());
        c->Write();
    }

    outFile->Close();
    cout << "\n=== Analysis Complete ===" << endl;
    cout << "Plots saved in 'outputs/03_amm_validation/' directory." << endl;
}
